...
const tempHtml = path.join(pluginRoot, 'data', 'measure.html');
if (!fs.existsSync(path.join(pluginRoot, 'data'))) fs.mkdirSync(path.join(pluginRoot, 'data'), { recursive: true });
...
(async () => {
  ...
  const data = await page.evaluate(() => {
    const circles = document.querySelectorAll('.circle-row .li');
    const circleData = [];
    circles.forEach((li) => {
      const box = li.querySelector('.container-box');
      const summary = li.querySelector('summary');
      circleData.push({
        label: summary ? summary.textContent : '',
        center: Math.round(box.getBoundingClientRect().left + box.getBoundingClientRect().width / 2)
      });
    });
    const nameCard = document.querySelector('.card[style*="margin-left"]');
    return {
      circles: circleData,
      nameCardLeft: nameCard ? Math.round(nameCard.getBoundingClientRect().left) : null,
      nameCardWidth: nameCard ? Math.round(nameCard.getBoundingClientRect().width) : null
    };
  });

  console.log(JSON.stringify(data, null, 2));
  const eng = data.circles.find(c => c.label === '体力');
  const pain = data.circles.find(c => c.label === '疼痛');
  if (eng && pain) {
    const midX = (eng.center + pain.center) / 2;
    console.log('体力/疼痛中间 x:', midX);
    console.log('名称卡片左边缘 x:', data.nameCardLeft);
    console.log('偏差:', data.nameCardLeft - midX);
  }
  await browser.close();
})();